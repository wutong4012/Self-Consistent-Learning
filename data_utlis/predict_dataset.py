...
def gen_postprocess(output_dict, gen_tokenizer, config):
    ...
    if config.cycle >= 0:
        gen_ds_dict = multiply_pre_score(config, train_ds)
...
def gen_pred_collate(batch_data, gen_tokenizer):
    ...
    input_ids = pad_sequence(
        [x for x in input_ids], batch_first=True, padding_value=gen_tokenizer.pad_token_id)
...
def process_gen_ds(config):
    """
        使用Generator生成的句子再作为预测的输入句子
    """
    gen_ds = datasets.load_from_disk(config.sim_data_path + f'/trainG_cycle_{config.cycle}')
    if gen_ds.num_rows > config.sentence_num:
        random_list = random.sample(range(gen_ds.num_rows), config.sentence_num)
...
def create_predict_dataloader(config, tokenizer, attri):
    if attri == 'gen':
        batch_size = config.pre_gen_bs

        test_ds = datasets.Dataset.from_json(config.data_path + '/new_unlabel.json')
        ...
        if config.cycle == -1:
            ...
            extra_ds = process_gen_ds(config)
        if config.cycle != -1:
            sentence_ds = datasets.concatenate_datasets([origin_ds, extra_ds])
        print(f'**********The Test_ds Range is {config.start} ~~ {config.end}**********')
        
        predict_dataset = SimGanDataset(sentence_ds, tokenizer=tokenizer, predict=True, is_gen=True)

        def collate_fn(batch_data):
            return gen_pred_collate(batch_data, tokenizer)

    elif attri == 'dis':
        batch_size = config.pre_dis_bs

        gen_ds = load_data(config, is_score=True, attri='dis')
